...
function invCardHTML(inv) {
  ...
  return `<div class="inv-swipe-wrap" id="swipe-wrap-${inv.id}" style="margin-bottom:2px">
    ...
    <div class="inv-card ${cardStatus}${isSelected?' selected':''}" id="inv-card-${inv.id}" data-id="${inv.id}" style="background:${cardStatus==='status-lunas'?'var(--success-soft)':(cardStatus==='status-dp'?'var(--warning-soft)':\'var(--bg-card)\')}">
...
function saveSign() {
  ...
  const s = DB.get('settings',{}); s.signature = canvas.toDataURL('image/png'); DB.set('settings',s);
